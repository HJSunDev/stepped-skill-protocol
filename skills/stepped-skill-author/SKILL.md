...
3. 设计一条有限线性的 step 链，边界必须来自真实阶段。
4. 先把 `SKILL.md` 写成完整可用的普通 Skill fallback，再加入简短 SSP 协议胶囊和 `metadata.stepped-skill.*` 字段。
5. 在 `steps/` 下编写 step 文件，确保每个 step 足以独立完成当前阶段。
6. 面向发布的包，应根据 source 文件和便携投影规则创建或检查 `.ssp/manifest.json`。
7. 有本地 validator 时把它作为可选确认；没有 validator 时，按内置检查表手动验证。
8. 先修复结构错误，再润色表达。
